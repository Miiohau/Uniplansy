...
class ThreadLocalGuidSupplier(GUIDSupplier):
    """a GuidSupplier that uses a thread-local sequence number and thread_ids to create unique guids. Note:this GuidSupplier simple on purpose to run fast however because of that is unsuitable for long-running applications because it will generate increasing large guids."""
    ...
    def create_guid(self, prefix: str = "") -> str:
        """creates a guid in the context of the current thread. Note: assumes if the current thread is named that name is globally unique (you can create globally unique thread names via the create_thread_guid function)."""
